...
class StudentForm(forms.ModelForm):
    password1 = forms.CharField(
        ...
        help_text='Password must be at least 8 characters long and contain numbers, uppercase and lowercase letters.',
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Make password fields required only for new students
    ...
    def save(self, commit=True):
        ...
        if not student.pk:
            ...
            base_username = f"STD{self.cleaned_data['admission_number']}"
            ...
            # Store the password before hashing for display
...
class TeacherForm(forms.ModelForm):
    password1 = forms.CharField(
        ...
        help_text='Minimum 4 characters required'
    )
    password2 = forms.CharField(
        label='Confirm Password',
        widget=forms.PasswordInput(attrs={'class': 'form-control'})
    ...
    class Meta:
        ...
        fields = [
            ...
            'profile_picture', 'status', 'date_of_birth',
            'gender', 'marital_status', 'years_of_experience',
            'location', 'grade', 'is_class_teacher', 'certificate', 'branch',
    ...
    def clean(self):
        ...
        required_fields = ['position', 'first_name', 'last_name', 'email']
...
class SMSMessageForm(forms.ModelForm):
    class Meta:
        model = SMSMessage
        fields = ['message', 'recipient_type', 'specific_grade', 'specific_student']
        widgets = {
...
class AdminStaffForm(forms.ModelForm):
    password1 = forms.CharField(
        label='Password',
        widget=forms.PasswordInput(attrs={'class': 'form-control'}),
        help_text='Create a strong password for the admin account'
    )
    password2 = forms.CharField(
        label='Confirm Password',
        widget=forms.PasswordInput(attrs={'class': 'form-control'})
