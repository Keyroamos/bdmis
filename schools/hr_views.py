...
def api_hr_staff_list(request):
    ...
    try:
        staff = NonTeachingStaff.objects.all().select_related('supervisor')
        ...
        try:
            ...
            drivers = TransportDriver.objects.all()
            for d in drivers:
                staff_data.append({
                    'id': d.id,
                    'is_teacher': False,
...
def api_hr_leave_requests(request):
    ...
    try:
        leaves = Leave.objects.all().select_related('employee').prefetch_related('approvals')
...
def api_hr_advance_requests(request):
    """Get all salary advance requests with approval status"""
    try:
        advances = SalaryAdvance.objects.all().select_related('employee').prefetch_related('approvals')
...
def api_supervisors_list(request):
    """Get all supervisors with their team statistics"""
    try:
        # Get all employees who have supervised staff
