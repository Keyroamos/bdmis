...
def generate_payment_receipt(payment, qr_code=None):
    ...
    accent_color = colors.HexColor('#f5f7fa')  # Light background
    light_bg = colors.HexColor('#f5f7fa')  # Light background for sections
    ...
    # 1. Custom Uploaded Logo (Priority)
    if sc.school_logo:
        try:
            # Get absolute filesystem path
            possible_logo_paths.append(sc.school_logo.path)
        except Exception:
            pass # Handle cloud storage or other path issues gracefully
    ...
    for path in possible_logo_paths:
        if os.path.exists(path):
            logo_path = path
            try:
                logo_image = Image(logo_path, width=40*mm, height=40*mm, kind='proportional')
                break
            except Exception as e:
                print(f"Error loading logo from {path}: {e}")
                logo_image = None
    
    # School Header with Logo - Modern gradient design
    # --- Header Layout Matching Web View ---
    
    # Left Side: School Info
    school_info_table = Table([
        [Paragraph(sc.school_name.upper(), school_name_style)],
        [Paragraph(f"{sc.school_address}", contact_style)],
        [Paragraph(f"Tel: {sc.school_phone} | {sc.school_email}", contact_style)]
    ], colWidths=[100*mm])
    ...
    # Combine Logo + School Info
    if logo_image:
        left_header_data = [[logo_image, school_info_table]]
        left_header = Table(left_header_data, colWidths=[25*mm, 85*mm])
        left_header.setStyle(TableStyle([
            ('VALIGN', (0, 0), (-1, -1), 'TOP'),
            ('LEFTPADDING', (0, 0), (-1, -1), 0),
        ]))
    else:
        left_header = school_info_table

    # Right Side: Receipt Meta
    receipt_meta_data = [
        [Paragraph("RECEIPT", ParagraphStyle('ReceiptBadge', parent=styles['Normal'], fontSize=16, textColor=colors.HexColor('#667eea'), fontName='Helvetica-Bold', alignment=2, backColor=colors.HexColor('#eef2ff'), borderWidth=1, borderColor=colors.HexColor('#e0e7ff'), padding=5, borderRadius=5))],
        [Spacer(1, 5)],
        [Paragraph("Receipt Number", ParagraphStyle('MetaLabel', parent=label_style, alignment=2, textColor=colors.HexColor('#a0aec0')))],
        [Paragraph(f"#{str(payment.reference_number)}", ParagraphStyle('MetaValue', parent=value_style, alignment=2, fontSize=12, fontName='Helvetica-Bold'))],
        [Spacer(1, 2)],
        [Paragraph("Date Issued", ParagraphStyle('MetaLabel', parent=label_style, alignment=2, textColor=colors.HexColor('#a0aec0')))],
        [Paragraph(payment_date.strftime('%B %d, %Y'), ParagraphStyle('MetaValue', parent=value_style, alignment=2))],
    ]
    
    right_header = Table(receipt_meta_data, colWidths=[65*mm])
    ...
    main_header_data = [[left_header, right_header]]
    main_header = Table(main_header_data, colWidths=[115*mm, 65*mm])
    main_header.setStyle(TableStyle([
        ('VALIGN', (0, 0), (-1, -1), 'TOP'),
        ('TOPPADDING', (0, 0), (-1, -1), 10),
        ('BOTTOMPADDING', (0, 0), (-1, -1), 15),
        ('LINEBELOW', (0, 0), (-1, -1), 2, colors.HexColor('#f1f5f9')), # Subtle separator
    ]))
    
    # Wrapper with Purple Background for logo section? No, web view uses white bg with purple logo box.
    # We will replicate the style: White background, Logo in colored box.
    
    # Adjust Logo to be in a box if present
    if logo_image:
        # Re-create logo image with a colored background if possible, or just place it in a cell with background
        pass # ReportLab Image flowable transparent background usually. 
    ...
    # For meal payments, show meal type instead of term
    if is_meal_payment:
        term_display = payment.get_meal_type_display()
        term_label = "Meal Type:"
    else:
        term_display = f"Term {payment_term}"
        term_label = "Term:"
    
    class_term = Table([
        [Paragraph("Class / Grade", label_style), 
         Paragraph(str(payment.student.grade) if payment.student.grade else "", value_style),
         Paragraph(term_label, label_style), 
    ...
    # Amount in words
    amount_words = number_to_words(float(payment.amount))
    ...
    # Calculate payment breakdown - using term fees as main item
    if is_meal_payment:
        # For meal payments, get all payments in the group if grouped
        from schools.models import StudentMealPayment
        if payment.payment_group:
            ...
            meal_types = [payment.get_meal_type_display()]
        amount_paid = float(total_amount)
    else:
        term_fees = float(payment.student.get_term_fees() if hasattr(payment.student, 'get_term_fees') else payment.amount)
        amount_paid = float(payment.amount)
        all_payments = None
        meal_types = None
    
    # Check if this is an admission fee payment (KES 500)
    ADMISSION_FEE = 500.00
    is_admission_fee = not is_meal_payment and abs(amount_paid - ADMISSION_FEE) < 0.01  # Allow for floating point precision
    ...
    # Add payment items - Meal Payment, Admission Fee, or Tuition Fee
    if amount_paid > 0:
        shillings = int(amount_paid)
        cents = round((amount_paid - shillings) * 100)
        
        if is_meal_payment:
            # This is a meal payment - show meal type(s)
            if len(meal_types) > 1:
                # Multiple meals - show as "Meal Payment (All Meals)"
                payment_items.append([
                    Paragraph(f"Meal Payment ({', '.join(meal_types)})", table_cell_style),
                ...
                # Single meal
                payment_items.append([
                    Paragraph(f"Meal Payment - {meal_types[0]}", table_cell_style),
                    Paragraph(f"{shillings:,}", table_cell_style),
                    Paragraph(f"{cents:02d}", table_cell_style)
                ])
        ...
        elif is_admission_fee:
            # This is an admission fee payment
            payment_items.append([
                Paragraph("Admission Fee", table_cell_style),
                Paragraph(f"{shillings:,}", table_cell_style),
                Paragraph(f"{cents:02d}", table_cell_style)
            ])
            # Add empty rows for other items (excluding Admission Fee since it's already added)
            for item in ["Tuition Fee", "School Uniform", "Porridge / Food", "Exam Fee", 
                         "Stationery", "Assessment Book", "Diary", "Trip", "Transport", "Others"]:
                payment_items.append([
                    Paragraph(item, table_cell_style),
                    Paragraph("", table_cell_style),
            ...
            # This is a regular payment (tuition fee)
            payment_items.append([
                Paragraph("Tuition Fee", table_cell_style),
                Paragraph(f"{shillings:,}", table_cell_style),
                Paragraph(f"{cents:02d}", table_cell_style)
            ])
            # Add empty rows for other items (as shown in image)
            for item in ["Admission Fee", "School Uniform", "Porridge / Food", "Exam Fee", 
                         "Stationery", "Assessment Book", "Diary", "Trip", "Transport", "Others"]:
                payment_items.append([
                    Paragraph(item, table_cell_style),
                    Paragraph("", table_cell_style),
    ...
    if payment.transaction_id:
        payment_method_text += f" No. {payment.transaction_id}"
    ...
    # Fees Balance (Only if > 0, displayed in Red like Web View)
    balance = float(payment.student.get_balance() if hasattr(payment.student, 'get_balance') else 0)
    
    if balance > 0:
        # Append to payment items for consistent grid look, OR create a specific highlight box
        # Web view appends it to footer of table. 
        # We can append to payment_items before creating Table.
        
        # Add Balance Row
        payment_items.append([
            Paragraph("OUTSTANDING BALANCE", ParagraphStyle('BalLabel', parent=table_cell_style, fontName='Helvetica-Bold', textColor=colors.HexColor('#f56565'))),
            Paragraph("", table_cell_style), # Spacer
            Paragraph(f"KES {balance:,.2f}", ParagraphStyle('BalValue', parent=table_cell_style, fontName='Helvetica-Bold', textColor=colors.HexColor('#e53e3e'), alignment=2))
        ])
        
        # We need to update the style for this last row to have red background or similar?
        # Web view has: bg-red-50 text-red-600
        # We'll rely on text color for now in PDF standard.

    # QR Code and Footer (acts as unique barcode/verification at bottom of receipt)
    footer_data = []
    if qr_code:
        try:
            ...
            logger.error(f"Error adding QR code to payment receipt: {e}")
            footer_data = [
                [
                    Paragraph(
    ...
    # Signatures
    signatures = Table([
        [Paragraph("Signed by:", label_style), Paragraph("Authorized Signature:", label_style)],
        [Paragraph("_" * 20, value_style), Paragraph("_" * 20, value_style)],
        [Paragraph("System Generated", ParagraphStyle('SignTitle', parent=value_style, fontSize=7, textColor=colors.HexColor('#a0aec0'))), 
         Paragraph("Finance Officer", ParagraphStyle('SignTitle', parent=value_style, fontSize=7, textColor=colors.HexColor('#a0aec0')))]
    ...
    # Create a compact footer container
    footer_container_data = [[footer_table]]
    footer_container = Table(footer_container_data, colWidths=[180*mm])
