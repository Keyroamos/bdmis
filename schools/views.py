    ...
    try:
        ...
        if user is not None and user.is_active:
            ...
            if not user.is_superuser:
                # Non-superusers must have a specific role
                if hasattr(user, 'student_profile'):
                    ...
                    # Staff users without specific profiles can access staff portals
                    # Default to finance portal for generic staff
                    role = 'staff'
                    redirect_url = '/finance-portal/dashboard'
                else:
                    # User has no specific role, is not staff, and is not superuser
                    return JsonResponse({'error': 'Access denied. You do not have permission to access this system.'}, status=403)
            ...
            return JsonResponse({
                ...
                'user': {
                    ...
                    # Add specific profile IDs to the response so frontend can use them if needed
...
def api_grades(request):
    ...
    try:
        grades = Grade.objects.select_related('class_teacher').all().order_by('name') # Adjust ordering if you have a rank field
        data = [{
            'id': g.id, 
            'name': g.name, 
...
def api_student_detail(request, pk):
    ...
    try:
        ...
        student = Student.objects.select_related('grade').get(pk=pk)
...
def api_student_create(request):
    ...
    try:
        ...
        if not data.get('admission_number'):
            ...
            admission_number = config.generate_admission_number(grade=grade)
        ...
        if form.is_valid():
            ...
            with transaction.atomic():
                ...
                import uuid
                payment_method = data.get('payment_method', 'CASH')
                amount = Decimal('500.00') # Admin fee
...
def api_schedule(request):
    ...
    try:
        ...
        schedules = Schedule.objects.select_related('teacher', 'subject', 'grade', 'teacher__user').all()
...
def api_school_config(request):
    """API endpoint for getting school configuration"""
    try:
        from config.models import SchoolConfig
        config = SchoolConfig.get_config()
...
def api_school_config_update(request):
    """API endpoint for updating school configuration"""
    ...
    try:
        ...
        fields = [
            'school_name', 'school_code', 'school_email', 
            'school_phone', 'school_address', 'admission_number_format',
            'current_term', 'default_currency'
        ]
        ...
        if 'current_year' in data and data['current_year']:
            try:
                config.current_year = int(data['current_year'])
                updated = True
            except: pass
            
        if 'school_logo' in files:
            print(f"DEBUG: New logo received: {files['school_logo'].name}")
...
def stk_push(request):
    try:
        ...
        unique_ref = f"MP{timestamp}{student_id}"

        # Initiate STK push
        checkout_request_id, error = initiate_stk_push(
            phone=phone,
            amount=amount,
            account_ref=account_ref,  # Keep account_ref for M-Pesa display
            description=description
        )
        ...
        # Create pending payment record with unique reference
        payment = Payment.objects.create(
            student=student,
            amount=amount,
            ...
            reference_number=unique_ref,  # Use the unique reference
