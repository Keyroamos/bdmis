...
def api_food_dashboard(request):
    try:
        ...
        total_billed = FoodStudentAccount.objects.aggregate(sum=Sum('total_billed'))['sum'] or 0
        total_collected = FoodStudentAccount.objects.aggregate(sum=Sum('total_paid'))['sum'] or 0
        outstanding = FoodStudentAccount.objects.filter(balance__gt=0).aggregate(sum=Sum('balance'))['sum'] or 0
        
        # New Stats
        active_subscribers = FoodSubscription.objects.filter(active=True).count()
        ...
        # Recent Transactions
        recent_tx = FoodTransaction.objects.select_related('account__student').order_by('-date')[:5]
        ...
        popular_plans_qs = FoodSubscription.objects.values('meal_item__name').annotate(
            count=Count('id')
        ).order_by('-count')[:5]
...
def api_food_students(request):
    ...
    try:
        ...
        students = Student.objects.all().select_related('grade', 'food_account').order_by('first_name')
