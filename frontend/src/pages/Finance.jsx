        ...
        <div className="p-4 md:p-8 max-w-[1400px] mx-auto space-y-6 min-h-screen bg-slate-50/50 dark:bg-slate-950/20">
            ...
            <AnimatePresence mode="wait">
                                    ...
                                    {chartData.length > 0 ? (
                                        <ResponsiveContainer width="100%" height="100%" minWidth={0}>
                                            <AreaChart data={chartData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                                                ...
                                                <Area type="monotone" dataKey="expenses" stroke="#f43f5e" strokeWidth={3} fill="url(#colorExpense)" />
                                            </AreaChart>
                                        </ResponsiveContainer>
                                    ) : (
                                ...
                                <div className="space-y-3">
                                    {(expenseDistribution || []).map((item, idx) => (
                                        <div key={idx} className="flex justify-between items-center text-xs font-medium">
                                            ...
                                            <span className="text-slate-900 dark:text-white uppercase">KES {item.value.toLocaleString()}</span>
                                        </div>
                                    ))}
                                </div>
                            </div>
                ...
                {activeTab === 'collections' && (
                    ...
                    >
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                            <StatCard title="Total Collected" value={`KES ${((summary.total_revenue || 0) / 1000).toFixed(1)}k`} subtext="Total revenue" icon={Wallet} color="bg-emerald-600" />
                            <StatCard title="Expected Fees" value={`KES ${((summary.expected_fees || 0) / 1000).toFixed(1)}k`} subtext="Active students" icon={Calendar} color="bg-blue-600" />
                            <StatCard title="Outstanding" value={`KES ${((summary.fees_arrears || 0) / 1000).toFixed(1)}k`} subtext="Unpaid fees" icon={TrendingDown} color="bg-red-600" />
                                        ...
                                        {(recentCollections || []).length > 0 ? (
                                            recentCollections.map((tx, idx) => (
                                                <tr key={idx} className="group hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition-colors">
                                                    ...
                                                    <td className="px-6 py-4 text-sm font-bold text-emerald-600">KES {tx.amount.toLocaleString()}</td>
                                                    <td className="px-6 py-4">
                                    ...
                                    <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                                        {(recentExpenses || []).map((exp, idx) => (
                                            <tr key={idx} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition-colors">
                                                ...
                                                <td className="px-6 py-4 text-sm font-bold text-rose-600">KES {exp.amount.toLocaleString()}</td>
                ...
                {activeTab === 'fee-structure' && (
                    <motion.div key="fee-structure" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
                        <div className="flex justify-between items-center bg-white dark:bg-slate-900 p-6 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm">
                            <div><h3 className="text-lg font-bold text-slate-900 dark:text-white tracking-tight">Structured Fees</h3><p className="text-xs font-medium text-slate-500">Configured fee rates by grade and term</p></div>
                            <Button variant="primary" className="h-9 px-4 text-xs font-semibold bg-indigo-600 border-none shadow-sm"><Plus size={14} className="mr-2" /> Add Structure</Button>
                        </div>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pb-20">
                            {Object.entries(groupedFees).map(([grade, terms]) => (
                                <div key={grade} className="bg-white dark:bg-slate-900 rounded-xl p-6 border border-slate-200 dark:border-slate-800 shadow-sm">
                                    <div className="flex justify-between items-start mb-6">
                                        <div>
                                            <h4 className="text-xl font-bold text-slate-900 dark:text-white tracking-tight">{grade}</h4>
                                            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1">Academic Year 2024</p>
                                        </div>
                                        {editingGrade === grade ? (
                                            <div className="flex gap-2">
                                                <button onClick={() => handleSaveGrade(grade)} className="p-2 rounded-lg bg-emerald-500 text-white shadow"><Save size={16} /></button>
                                                <button onClick={handleCancelEdit} className="p-2 rounded-lg bg-slate-100 dark:bg-slate-800 text-slate-500"><X size={16} /></button>
                                            </div>
                                        ) : (
                                            <button onClick={() => handleEditGrade(grade, terms)} className="p-2 rounded-lg bg-slate-50 dark:bg-slate-800 text-slate-500 hover:bg-slate-100 transition-all"><Edit2 size={16} /></button>
                                        )}
                                    </div>
                                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                                        {[1, 2, 3].map(term => (
                                            <div key={term} className="bg-slate-50/50 dark:bg-slate-800/30 rounded-lg p-4">
                                                <span className="text-[10px] font-bold text-slate-400 uppercase mb-3 block">Term {term}</span>
                                                <div className="space-y-3">
                                                    {(terms[term] || []).map(fee => (
                                                        <div key={fee.id} className="flex flex-col gap-0.5">
                                                            <span className="text-[9px] font-bold text-slate-400 uppercase">{fee.category}</span>
                                                            {editingGrade === grade ? (
                                                                <input
                                                                    type="number"
                                                                    value={editValues[fee.id] || 0}
                                                                    onChange={(e) => handleInputChange(fee.id, e.target.value)}
                                                                    className="w-full px-2 py-1 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded text-xs font-bold outline-none focus:ring-1 focus:ring-indigo-500"
                                                                />
                                                            ) : (
                                                                <span className="text-xs font-bold text-slate-700 dark:text-slate-300">KES {fee.amount.toLocaleString()}</span>
                                                            )}
                                                        </div>
                                                    ))}
                                                </div>
