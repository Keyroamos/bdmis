...
const HRStaff = () => {
    ...
    const handleDelete = async (staffMember) => {
        if (staffMember.is_teacher) {
            alert("Please use the Teachers module to manage teaching staff deletion.");
            return;
        }
        if (staffMember.is_driver) {
            alert("Please use the Transport module to manage driver deletion.");
            return;
        }
        if (window.confirm(`Are you sure you want to delete ${staffMember.first_name} ${staffMember.last_name}?`)) {
            try {
                await axios.delete(`/api/hr/staff/${staffMember.id}/delete/`);
                fetchStaff();
            } catch (error) {
