...
const StatCard = ({ title, value, subtext, icon: Icon, color, trend, delay, net }) => (
    <motion.div
        ...
        className="bg-white dark:bg-slate-900/50 backdrop-blur-xl p-6 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 group"
    >
...
const FinanceDashboard = () => {
    ...
    useEffect(() => {
        if (activeTab === 'fee-structure') fetchFeeStructures();
        ...
        <div className="p-4 md:p-8 max-w-[1400px] mx-auto space-y-6 min-h-screen bg-slate-50/50 dark:bg-slate-950/20 text-slate-900 dark:text-white">
            ...
            <div className="space-y-6 pb-2">
                ...
                <div className="flex flex-wrap items-center bg-slate-900/50 backdrop-blur-md p-1.5 rounded-2xl border border-slate-800 shadow-xl gap-1 overflow-x-auto no-scrollbar">
                    ...
                    <TabButton id="transport" label="Transport" icon={Truck} />
                    <TabButton id="food" label="Catering" icon={Utensils} />
                </div>
            </div>
            ...
            <div className="relative">
                <AnimatePresence mode="wait">
                                ...
                                <div className="lg:col-span-8 bg-white dark:bg-slate-900/40 backdrop-blur-md p-8 rounded-[2rem] border border-slate-200 dark:border-slate-800 shadow-sm relative overflow-hidden">
                                    ...
                                    <div className="h-[400px] w-full flex items-center justify-center">
                                        {chartData.length > 0 ? (
                                            ...
                                            <div className="text-slate-400 font-bold uppercase tracking-widest text-[10px]">No Data</div>
                                        )}
                                    </div>
                                </div>
                                <div className="lg:col-span-4 bg-white dark:bg-slate-900/40 p-8 rounded-[2rem] border border-slate-200 dark:border-slate-800 shadow-sm flex flex-col">
                                    ...
                                    <div className="flex-1 space-y-8">
                                        {[
                                            ...
                                            { name: 'Food', value: breakdown.food || 0, color: '#f59e0b' },
                    ...
                    {activeTab === 'fee-structure' && (
                        <motion.div key="fee-structure" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-6">
                            <div className="bg-slate-900 shadow-2xl rounded-[2rem] border border-slate-800 overflow-hidden">
                                <div className="p-8 border-b border-slate-800 flex justify-between items-center text-white">
                                    <div>
                                        <h3 className="text-2xl font-black">Structured Fees</h3>
                                        <p className="text-xs text-slate-500 font-bold uppercase tracking-widest mt-1">Rates by grade</p>
                                    </div>
                                    <div className="flex gap-3">
                                        {editMode ? (
                                            <>
                                                <Button onClick={() => { setEditMode(false); setEditingFees({}); }} variant="outline" className="h-10 text-xs font-bold border-slate-700 text-slate-400">Cancel</Button>
                                                <Button onClick={saveFeeUpdates} disabled={isSaving} className="h-10 text-xs font-bold bg-indigo-600 border-none">Save & Sync</Button>
                                            </>
                                        ) : (
                                            <Button onClick={() => setEditMode(true)} variant="outline" className="h-10 text-xs font-bold border-slate-700 text-indigo-400">Edit Fees</Button>
                                        )}
                                    </div>
                                </div>
                                <div className="p-8">
                                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                                        {Object.entries(feeGroups).map(([grade, terms]) => (
                                            <div key={grade} className="bg-slate-800/20 rounded-3xl border border-slate-800/50 p-6">
                                                <h4 className="text-2xl font-black text-white mb-6">{grade}</h4>
                                                <div className="grid grid-cols-3 gap-4">
                                                    {[1, 2, 3].map(tNum => (
                                                        <div key={tNum} className="space-y-4">
                                                            <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Term {tNum}</span>
                                                            <div className="space-y-3">
                                                                {(terms[tNum] || []).map(fee => (
                                                                    <div key={fee.id} className="p-3 bg-slate-900/50 rounded-2xl border border-slate-800/50">
                                                                        <p className="text-[9px] font-black text-slate-500 uppercase tracking-tighter mb-1.5">{fee.category}</p>
                                                                        {editMode ? (
                                                                            <input
                                                                                type="number"
                                                                                className="bg-transparent border-none focus:ring-0 text-xs font-black text-white w-full p-0 h-6"
                                                                                defaultValue={fee.amount}
                                                                                onChange={(e) => handleFeeChange(fee.id, e.target.value)}
                                                                            />
                                                                        ) : (
                                                                            <p className="text-xs font-black text-white">KES {fee.amount.toLocaleString()}</p>
                                                                        )}
                                                                    </div>
                                                                ))}
                                                            </div>
