...
import {
    ...
    Wallet, Receipt, History, Landmark, GraduationCap
} from 'lucide-react';
...
const StatCard = ({ label, value, icon: Icon, color, trend }) => (
    <div className={`p-4 md:p-5 rounded-2xl border ${color} bg-white dark:bg-slate-900/50 backdrop-blur-sm flex flex-col items-center text-center md:items-start md:text-left relative overflow-hidden group`}>
        <div className="flex justify-center md:justify-between items-center w-full mb-3 md:mb-4">
            <div className={`p-2.5 rounded-xl ${color.replace('border-', 'bg-').replace('200', '50')} dark:bg-opacity-20 group-hover:scale-110 transition-transform`}>
                <Icon size={20} className={color.replace('border-', 'text-').replace('200', '600')} />
            </div>
            {trend && (
                <span className="hidden md:flex items-center text-[10px] font-bold text-green-600 dark:text-green-400 bg-green-50 dark:bg-green-900/30 px-2 py-1 rounded-lg">
        ...
        <div className="space-y-1">
            <p className="text-slate-500 dark:text-slate-400 text-[10px] md:text-xs font-bold uppercase tracking-widest">{label}</p>
            <h3 className="text-xl md:text-2xl font-black text-slate-900 dark:text-white leading-none">{value}</h3>
            {trend && (
                <div className="md:hidden mt-2">
                    <span className="inline-flex items-center text-[10px] font-bold text-green-600 dark:text-green-400 bg-green-50 dark:bg-green-900/30 px-2 py-0.5 rounded-md">
                        {trend}
                    </span>
                </div>
            )}
...
const TabButton = ({ id, label, icon: Icon, active, onClick }) => (
    <button
        onClick={() => onClick(id)}
        className={`relative flex items-center gap-2 px-4 md:px-5 py-2 md:py-2.5 rounded-full text-xs md:text-sm font-medium transition-all shrink-0 ${active
            ? 'text-white'
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div className="bg-gradient-to-br from-blue-50 to-indigo-50 dark:from-blue-900/20 dark:to-indigo-900/20 rounded-xl p-4 border border-blue-100 dark:border-blue-800/30">
                    <div className="flex items-center gap-2 mb-2">
                        ...
                        <p className="text-xs text-blue-700 dark:text-blue-400 font-bold uppercase">Overall Average</p>
                    </div>
                    <p className="text-3xl font-black text-blue-900 dark:text-blue-300">{academicData.stats.average_score}%</p>
                </div>
                ...
                <div className="bg-gradient-to-br from-purple-50 to-pink-50 dark:from-purple-900/20 dark:to-pink-900/20 rounded-xl p-4 border border-purple-100 dark:border-purple-800/30">
                    <div className="flex items-center gap-2 mb-2">
                        ...
                        <p className="text-xs text-purple-700 dark:text-purple-400 font-bold uppercase">Assessments</p>
                    </div>
                    <p className="text-3xl font-black text-purple-900 dark:text-purple-300">{academicData.stats.total_assessments}</p>
                </div>
                ...
                <div className="bg-gradient-to-br from-emerald-50 to-teal-50 dark:from-emerald-900/20 dark:to-teal-900/20 rounded-xl p-4 border border-emerald-100 dark:border-emerald-800/30">
                    <div className="flex items-center gap-2 mb-2">
                        ...
                        <p className="text-xs text-emerald-700 dark:text-emerald-400 font-bold uppercase">Subjects</p>
                    </div>
                    <p className="text-3xl font-black text-emerald-900 dark:text-emerald-300">{academicData.stats.subjects_count}</p>
                </div>
                ...
                <div className="bg-gradient-to-br from-amber-50 to-orange-50 dark:from-amber-900/20 dark:to-orange-900/20 rounded-xl p-4 border border-amber-100 dark:border-amber-800/30">
                    <div className="flex items-center gap-2 mb-2">
                        ...
                        <p className="text-xs text-amber-700 dark:text-amber-400 font-bold uppercase">Latest Term</p>
                    </div>
                    <p className="text-xl font-black text-amber-900 dark:text-amber-300">{academicData.stats.latest_term}</p>
                </div>
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="bg-white dark:bg-slate-900 rounded-3xl p-6 border border-slate-100 dark:border-slate-800 shadow-sm hover:shadow-md transition-shadow">
                <div className="flex items-center justify-between mb-6">
                    ...
                    <Button variant="ghost" size="sm" className="h-8 w-8 p-0 rounded-full"><Edit size={14} /></Button>
                </div>
                <div className="space-y-4">
                    <div className="bg-slate-50 dark:bg-slate-800/50 p-4 rounded-2xl flex items-center gap-4">
                        <div className="h-12 w-12 rounded-full bg-purple-100 dark:bg-purple-900/30 text-purple-600 flex items-center justify-center font-bold text-lg">
                            {student.guardian.name ? student.guardian.name.charAt(0) : 'G'}
                        </div>
                        <div>
                            <p className="font-bold text-slate-900 dark:text-white">{student.guardian.name || 'No Guardian Name'}</p>
            ...
            <div className="bg-white dark:bg-slate-900 rounded-3xl p-6 border border-slate-100 dark:border-slate-800 shadow-sm hover:shadow-md transition-shadow">
                ...
                <div className="grid grid-cols-2 gap-4 mb-6">
                    <div className="p-4 rounded-2xl bg-blue-50 dark:bg-blue-900/10 text-center">
                        <p className="text-xs text-blue-600 dark:text-blue-400 font-bold uppercase mb-1">Grade</p>
                        <p className="text-2xl font-black text-slate-900 dark:text-white">{student.personal.grade}</p>
                    </div>
                    <div className="p-4 rounded-2xl bg-indigo-50 dark:bg-indigo-900/10 text-center">
                        <p className="text-xs text-indigo-600 dark:text-indigo-400 font-bold uppercase mb-1">Rank</p>
                        <p className="text-2xl font-black text-slate-900 dark:text-white">#{student.academic.position}</p>
                    </div>
                </div>
                <Button className="w-full justify-between group" variant="outline">
                    View Full Report
                    <ArrowUpRight size={16} className="text-slate-400 group-hover:text-primary-600" />
                </Button>
            </div>

            {/* Attendance Snapshot */}
            <div className="bg-gradient-to-br from-slate-900 to-slate-800 dark:from-slate-800 dark:to-slate-900 rounded-3xl p-6 text-white shadow-lg">
                <div className="flex items-center justify-between mb-6">
                ...
                <div className="flex items-center justify-center py-4">
                    ...
                    <div className="relative h-32 w-32 flex items-center justify-center">
                        ...
                        <div className="absolute flex flex-col items-center">
                            <span className="text-3xl font-bold">{student.attendance.attendance_rate}%</span>
    ...
    const renderFinancials = () => (
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-3 md:gap-6">
                <StatCard
                    label="Total Billed"
                    value={`$${student.financial.total_fees.toLocaleString()}`}
                    icon={CreditCard}
                    color="border-slate-200"
                />
                <StatCard
                    label="Amount Paid"
                    value={`$${student.financial.total_paid.toLocaleString()}`}
                    icon={CheckCircle}
                ...
                <StatCard
                    label="Outstanding"
                    value={`$${student.financial.balance.toLocaleString()}`}
                    icon={AlertCircle}
                    color={student.financial.balance > 0 ? "border-red-200 text-red-600" : "border-slate-200"}
                />
            </div>
                    ...
                    {student.financial.history.map((tx) => (
                        <div key={tx.id} className="p-4 space-y-3">
                            <div className="flex justify-between items-start">
                                <div>
                                    <p className="text-sm font-bold text-slate-900 dark:text-white">Term {tx.term} Fees</p>
                                    <p className="text-[10px] text-slate-500 font-medium">{tx.date}</p>
                                </div>
                                <div className="text-right">
                                    <p className="text-sm font-black text-slate-900 dark:text-white">${tx.amount.toLocaleString()}</p>
                                    <span className="inline-flex items-center px-1.5 py-0.5 rounded-md text-[9px] font-bold bg-green-50 text-green-700 dark:bg-green-900/30 dark:text-green-400 uppercase tracking-tighter">
                                        Success
                                    </span>
                                </div>
                            </div>
                            <div className="flex justify-between items-center text-[10px] bg-slate-50 dark:bg-slate-800/50 p-2 rounded-lg border border-slate-100 dark:border-slate-700/50">
                                <div className="flex items-center gap-2">
                                    <History size={12} className="text-slate-400" />
                                    <span className="font-mono text-slate-500 uppercase">{tx.reference}</span>
                                </div>
                        ...
                        <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                            {student.financial.history.map((tx) => (
                                <tr key={tx.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                                    ...
                                    <td className="px-6 py-4 text-sm font-bold text-right text-slate-900 dark:text-white">${tx.amount.toLocaleString()}</td>
        ...
        <div className="min-h-screen bg-slate-50 dark:bg-slate-950 pb-20">
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-8 py-8 space-y-8">
                ...
                <div className="relative">
                    {/* Background Banner */}
                    <div className="h-32 md:h-48 w-full bg-gradient-to-br from-slate-900 via-slate-800 to-primary-900 rounded-[2rem] overflow-hidden shadow-lg border border-slate-200/10 dark:border-white/5">
                        ...
                        {/* Decorative elements */}
                        <div className="absolute top-4 right-4 h-24 w-24 bg-primary-500/20 rounded-full blur-[40px]"></div>
                        <div className="absolute bottom-4 left-4 h-32 w-32 bg-secondary-500/10 rounded-full blur-[50px]"></div>
                    </div>

                    {/* Profile Overlay */}
                    <div className="relative -mt-16 md:-mt-20 px-4 md:px-10 flex flex-col md:flex-row items-center md:items-end gap-4 md:gap-8">
                        {/* Profile Photo */}
                        <div className="relative group">
                            <div className="h-32 w-32 md:h-44 md:w-44 rounded-[2rem] border-[6px] border-white dark:border-slate-950 bg-white dark:bg-slate-900 shadow-2xl overflow-hidden p-1 flex items-center justify-center transform transition-transform group-hover:scale-[1.02]">
                                {student.personal.photo ? (
                                    ...
                                    <div className="w-full h-full bg-slate-50 dark:bg-slate-800/50 rounded-[1.6rem] flex items-center justify-center text-slate-300">
                                        <User size={64} />
                                    </div>
                                )}
                            </div>
                            <button className="absolute bottom-1 right-1 md:bottom-3 md:right-3 p-2.5 bg-primary-600 text-white rounded-2xl shadow-xl hover:bg-primary-700 transition-all hover:scale-110 border-4 border-white dark:border-slate-950">
                                <Edit size={16} />
                            </button>
                        </div>

                        {/* Profile Info Card Content */}
                        <div className="pb-2 md:pb-6 flex-1 flex flex-col items-center md:items-start text-center md:text-left w-full md:w-auto mt-2 md:mt-0">
                            <div className="flex flex-col md:flex-row md:items-center gap-2 md:gap-3 mb-3">
                                ...
                                {student.financial.balance <= 0 && (
                                    <div className="flex justify-center md:justify-start">
                                        <span className="inline-flex items-center px-4 py-1.5 rounded-full text-[10px] md:text-xs font-black bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border border-emerald-500/20 shadow-sm backdrop-blur-md">
                ...
                {/* Tabs */}
                <div className="flex md:justify-start overflow-x-auto pb-4 md:pb-0 no-scrollbar -mx-4 px-4 sm:mx-0 sm:px-0">
                    <div className="flex p-1 bg-white dark:bg-slate-900 rounded-full border border-slate-200 dark:border-slate-800 shadow-sm min-w-max">
                                        ...
                                        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                                            <div className="bg-gradient-to-br from-emerald-50 to-teal-50 dark:from-emerald-900/20 dark:to-teal-900/20 rounded-xl p-4 border border-emerald-100 dark:border-emerald-800/30">
                                                <div className="flex items-center gap-3">
                                                    <div className="h-10 w-10 bg-emerald-500 rounded-lg flex items-center justify-center shrink-0">
                                                        <CheckCircle size={18} className="text-white" />
                                                    </div>
                                                    <div>
                                                        <p className="text-xs text-emerald-700 dark:text-emerald-400 font-bold uppercase">Rate</p>
                                                        <p className="text-2xl font-black text-emerald-900 dark:text-emerald-300">{attendanceStats.rate}%</p>
                                                    </div>
                                                </div>
                                            </div>
                                            ...
                                            <div className="bg-gradient-to-br from-blue-50 to-indigo-50 dark:from-blue-900/20 dark:to-indigo-900/20 rounded-xl p-4 border border-blue-100 dark:border-blue-800/30">
                                                <div className="flex items-center gap-3">
                                                    <div className="h-10 w-10 bg-blue-500 rounded-lg flex items-center justify-center shrink-0">
                                                        <Calendar size={18} className="text-white" />
                                                    </div>
                                                    <div>
                                                        <p className="text-xs text-blue-700 dark:text-blue-400 font-bold uppercase">Present</p>
                                                        <p className="text-2xl font-black text-blue-900 dark:text-blue-300">{attendanceStats.present}</p>
                                                    </div>
                                                </div>
                                            </div>
                                            ...
                                            <div className="bg-gradient-to-br from-rose-50 to-pink-50 dark:from-rose-900/20 dark:to-pink-900/20 rounded-xl p-4 border border-rose-100 dark:border-rose-800/30">
                                                <div className="flex items-center gap-3">
                                                    <div className="h-10 w-10 bg-rose-500 rounded-lg flex items-center justify-center shrink-0">
                                                        <XCircle size={18} className="text-white" />
                                                    </div>
                                                    <div>
                                                        <p className="text-xs text-rose-700 dark:text-rose-400 font-bold uppercase">Absent</p>
                                                        <p className="text-2xl font-black text-rose-900 dark:text-rose-300">{attendanceStats.absent}</p>
                                                    </div>
                                                </div>
                                            </div>
                                            ...
                                            <div className="bg-gradient-to-br from-amber-50 to-orange-50 dark:from-amber-900/20 dark:to-orange-900/20 rounded-xl p-4 border border-amber-100 dark:border-amber-800/30">
                                                <div className="flex items-center gap-3">
                                                    <div className="h-10 w-10 bg-amber-500 rounded-lg flex items-center justify-center shrink-0">
                                                        <Clock size={18} className="text-white" />
                                                    </div>
                                                    <div>
                                                        <p className="text-xs text-amber-700 dark:text-amber-400 font-bold uppercase">Late</p>
                                                        <p className="text-2xl font-black text-amber-900 dark:text-amber-300">{attendanceStats.late}</p>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
