...
const TeacherCreate = () => {
    ...
    const [formData, setFormData] = useState({
        ...
        address: '',
        password1: '', password2: '', // Account Credentials
    ...
    const validateStep = (currentStep) => {
        const required = {
            1: ['first_name', 'last_name', 'email', 'national_id', 'date_of_birth', 'password1'],
            2: ['tsc_number', 'position'],
            3: ['basic_salary', 'branch']
        };
    ...
    const handleNext = () => {
        if (validateStep(step)) {
            if (step === 1 && formData.password1 !== formData.password2) {
                setError('Passwords do not match');
                return;
            }
            if (step === 1 && formData.password1.length < 4) {
                setError('Password must be at least 4 numeric digits (or characters)');
                return;
            }
            setStep(prev => prev + 1);
            setError(null);
                    ...
                    <div className="min-h-[400px]">
                        <AnimatePresence mode="wait">
                                    ...
                                    <div className="lg:col-span-2 space-y-6">
                                        ...
                                        <div className="bg-blue-50 dark:bg-slate-700/50 p-6 rounded-2xl border border-blue-100 dark:border-slate-600">
                                            <div className="flex items-center gap-2 mb-4">
                                                ...
                                                <h3 className="font-bold text-slate-800 dark:text-white">Account Credentials</h3>
                                            </div>
                                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                                <Input type="password" label="Create Password *" name="password1" value={formData.password1} onChange={handleChange} required placeholder="xxxx" />
                                                <Input type="password" label="Confirm Password *" name="password2" value={formData.password2} onChange={handleChange} required placeholder="xxxx" />
                                            </div>
                                            <div className="mt-4 p-3 bg-yellow-50 text-yellow-700 text-sm rounded-xl border border-yellow-100">
                                                <span className="font-semibold">Password Policy:</span> Exactly 4 numeric digits (e.g. 1234) for easy login.
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div className="space-y-6">
                                        ...
                                        <div className="bg-blue-50 dark:bg-blue-900/10 p-5 rounded-2xl">
                                            ...
                                            <p className="text-sm text-blue-700 dark:text-blue-400 leading-relaxed">
                                                ...
                                                <br /><br />
                                                The teacher uses their <b>email address</b> as username.
                                            </p>
                                        </div>
                                    </div>
                                </motion.div>
                            ...
                            {step === 2 && (
                                ...
                                >
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                        <Input label="TSC Number" name="tsc_number" value={formData.tsc_number} onChange={handleChange} required />
                                        <div className="space-y-2">
                                    ...
                                    <div className="space-y-3">
                                        ...
                                        <select
                                            ...
                                            className="w-full h-40 rounded-xl border border-slate-200 bg-slate-50 dark:bg-slate-900 px-4 py-3 focus:ring-2 focus:ring-secondary-100 focus:border-secondary-500 transition-all outline-none scrollbar-thin"
                                        >
                                            {subjects.map(s => (
                                    ...
                                    <div className="bg-slate-50 p-6 rounded-2xl border border-slate-100">
                                        <div className="flex items-center justify-between mb-4">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <span className="font-medium text-slate-700">Designate as Class Teacher</span>
                                            </div>
                                        </div>

                                        {formData.is_class_teacher && (
                                            <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }}>
                                                <p className="text-sm text-slate-500 mb-2">Enable this to assign this teacher to a specific grade and section</p>
                                                <select name="grade" value={formData.grade} onChange={handleChange} className="w-full md:w-1/2 h-11 rounded-xl border border-slate-200 px-4 focus:ring-2 focus:ring-secondary-100 outline-none">
                            ...
                            {step === 3 && (
                                ...
                                >
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                        ...
                                        <div className="space-y-2">
                                            <label className="text-sm font-medium text-slate-700 dark:text-slate-300 ml-1">Campus Location *</label>
                                            <select name="branch" value={formData.branch} onChange={handleChange} className="w-full h-11 rounded-xl border border-slate-200 bg-slate-50 dark:bg-slate-900 dark:border-slate-700 dark:text-white px-4 focus:ring-2 focus:ring-secondary-100 focus:border-secondary-500 transition-all outline-none" required>
                                    ...
                                    <div className="space-y-4">
                                        ...
                                        <div className="border-2 border-dashed border-slate-200 dark:border-slate-700 rounded-2xl p-8 flex flex-col items-center justify-center text-center cursor-pointer hover:border-secondary-400 transition-colors bg-slate-50 dark:bg-slate-900/50">
                                            <div className="w-16 h-16 bg-blue-50 text-blue-500 rounded-full flex items-center justify-center mb-4">
                                                <FileText size={32} />
                                            </div>
                                            <p className="text-sm text-slate-600 mb-1 font-medium">{formData.certificate ? formData.certificate.name : 'Choose file or drag & drop'}</p>
                                            <p className="text-xs text-slate-400">PDF or Image of highest qualification</p>
                                            <input type="file" name="certificate" onChange={handleChange} className="absolute inset-0 opacity-0 cursor-pointer w-full h-full" style={{ position: 'relative', display: 'none' }} id="cert-upload" />
                                            <Button variant="outline" size="sm" className="mt-4" onClick={() => document.getElementById('cert-upload').click()}>
                                                {formData.certificate ? 'Change File' : 'Browse Files'}
                                            </Button>
                                            {/* Fix for file input triggering */}
                                            <label htmlFor="cert-upload" className="absolute inset-0 cursor-pointer"></label>
                                        </div>
                                    </div>
