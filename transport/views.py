...
def api_transport_dashboard(request):
    try:
        ...
        total_billed = TransportStudentAccount.objects.aggregate(sum=Sum('total_billed'))['sum'] or 0
        total_collected = TransportStudentAccount.objects.aggregate(sum=Sum('total_paid'))['sum'] or 0
        outstanding = TransportStudentAccount.objects.filter(balance__gt=0).aggregate(sum=Sum('balance'))['sum'] or 0
        
        # New Stats
        active_students = TransportStudentAccount.objects.filter(active=True).count()
        
        today = timezone.now().date()
        todays_revenue = TransportTransaction.objects.filter(
            type='PAYMENT', date__date=today
        ).aggregate(sum=Sum('amount'))['sum'] or 0

        # Recent Transactions
        recent_tx = TransportTransaction.objects.select_related('account__student').order_by('-date')[:5]
        ...
        # Popular Routes
        popular_routes_qs = TransportAssignment.objects.values('route__name').annotate(
            count=Count('id')
        ).order_by('-count')[:5]
...
def api_transport_students(request):
    ...
    """
    try:
        students = Student.objects.all().select_related('grade', 'transport_account').order_by('first_name')
